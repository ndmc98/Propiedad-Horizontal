...
public class ApartamentoDAO {
    ...
    public void actualizarApartamento() throws CaException {
        try {
            String strSQL = "UPDATE Apartamento SET q_area = ?, q_piso = ?, t_coeficiente = ? WHERE k_idapartamento = ? ";
            Connection conexion = ServiceLocator.getInstance().tomarConexion();
            try (PreparedStatement prepStmt = conexion.prepareStatement(strSQL)) {
     ...
     * @return JComboBox
     */
    public JComboBox<String> listarApartamento(int k_idbloquee, int k_codigo) throws CaException {
        try {
            final JComboBox<String> listaApartamento = new JComboBox<>();
            String strSQL = "SELECT a.k_idapartamento FROM Apartamento a, Bloquee b WHERE a.k_idbloquee = b.k_idbloquee AND b.k_codigo = ? AND b.k_idbloquee = ?";
            Connection conexion = ServiceLocator.getInstance().tomarConexion();
            PreparedStatement prepStmt = conexion.prepareStatement(strSQL);
            prepStmt.setInt(1, k_codigo);
            prepStmt.setInt(2, k_idbloquee);
            ResultSet rs = prepStmt.executeQuery();
            while (rs.next()) {
                listaApartamento.addItem(Integer.toString(rs.getInt(1)));
            }
