    ...
    public void actualizarApartamento() throws CaException {
        try {
            String strSQL = "UPDATE Apartamento SET q_area = ?, q_piso = ?, t_coeficiente = ?,"
                    + " k_identificacion = ?, k_idbloquee = ?  WHERE k_idapartamento = ?";
            Connection conexion = ServiceLocator.getInstance().tomarConexion();
            PreparedStatement prepStmt = conexion.prepareStatement(strSQL);
            ...
            prepStmt.setLong(4,apartamento.getK_identificacion());
            prepStmt.setInt(5,apartamento.getK_idbloque());
            prepStmt.setInt(6,apartamento.getK_idapartamento());
            prepStmt.executeUpdate();
            prepStmt.close();
            ServiceLocator.getInstance().commit();
