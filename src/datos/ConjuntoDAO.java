...
public class ConjuntoDAO {
    ...
    public JComboBox<String> listarConjuntos() throws CaException {
        try {
            final JComboBox<String> listaConjuntos = new JComboBox<>();
     ...
     * @throws CaException
     */
    public void actualizarConjunto() throws CaException {
        try {
            String strSQL = "UPDATE Conjunto SET n_nombre = ? ,v_mensual = ? ,t_descuento = ? ,q_cuentabancaria = ? ,q_diasdescuento = ?,t_mora = ? WHERE k_codigo = ? ";
            Connection conexion = ServiceLocator.getInstance().tomarConexion();
            try (PreparedStatement prepStmt = conexion.prepareStatement(strSQL)) {
                prepStmt.setString(1, conjunto.getN_nombre());
                prepStmt.setInt(2, conjunto.getV_mensual());
                prepStmt.setFloat(3, conjunto.getT_descuento());
                prepStmt.setInt(4, conjunto.getQ_cuentabancaria());
                prepStmt.setInt(5, conjunto.getQ_diasdescuento());
                prepStmt.setFloat(6, conjunto.getT_mora());
                prepStmt.setInt(7, conjunto.getK_codigo());
                prepStmt.executeUpdate();
            }
            ServiceLocator.getInstance().commit();
